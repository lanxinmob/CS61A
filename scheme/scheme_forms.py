...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        if isinstance(expressions.rest,Pair):
            ...
            env.bindings[signature] = expressions.rest
        return signature
        # END PROBLEM 4
        ...
        v = LambdaProcedure(signature.rest, expressions.rest,env)
        env.define(signature.first,v)
...
def do_and_form(expressions, env):
    ...
    b = expressions
    while b.rest != nil:
      if is_scheme_false(scheme_eval(b.first,env)):
         return False
      b = b.rest
    return scheme_eval(b.first,env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    b = expressions
    while b.rest != nil:
      re = scheme_eval(b.first,env)
      if is_scheme_true(re):
         return re
      b = b.rest
    return scheme_eval(b.first,env)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while c !=nil:
        a = c.first
        validate_form(a,2,2)
        names = Pair(a.first,names)
        vals = Pair(scheme_eval(a.rest.first,env),vals)
        c = c.rest
    def reverse(c):
        res = nil
        while c !=nil:
            res = Pair(c.first,res)
            c = c.rest
        return res
    validate_formals(names)
    # END PROBLEM 14
    return env.make_child_frame(reverse(names),reverse( vals))
