...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container==False and self.is_container :
               self.store_ant(place.ant)
               place.ant = self
            elif place.ant.is_container and place.ant.can_contain(self) and self.is_container==False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        time = 0
        _place = self.place
        while not _place.is_hive:
         if _place.bees!=[] and  time<= self.upper_bound and time>=self.lower_bound:
          return random_bee(_place.bees) # REPLACE THIS LINE
         else: _place = _place.entrance
         time +=1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees!=[] and self.turns_to_chew==0 :
           bee = random_bee(self.place.bees)
           bee.reduce_health(bee.health)
           self.turns_to_chew = self.chewing_turns+1
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while _place!=None: 
            if _place.ant !=None: 
               if _place.ant.is_container and _place.ant.ant_contained!=None:
                  if _place.ant.mode==False:
                      _place.ant.double() 
                      _place.ant.mode=True
                  if _place.ant.ant_contained.mode==False:
                      _place.ant.ant_contained.double()
                      _place.ant.ant_contained.mode=True
               else: 
                    if _place.ant.mode==False:
                     _place.ant.double() 
                     _place.ant.mode=True
