...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_tor = length(left(m))*total_mass(end(left(m)))
        right_tor = length(right(m))*total_mass(end(right(m)))
        return bool(left_tor == right_tor and balanced(end(left(m))) and balanced(end(right(m))))
...
def max_path_sum(t):
    ...
    if not is_leaf(t):
       a = branches(t)
       return label(t)+max(max_path_sum(a[0]),max_path_sum(a[1]))
    else:
       return label(t)
