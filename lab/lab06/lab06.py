...
class Account:
    ...
    def deposit(self, amount):
        ...
        self.transactions.append(Transaction(self.id+1,self.balance-amount,self.balance))
        self.id+=1
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            self.transactions.append(Transaction(self.id+1,self.balance,self.balance))
            self.id+=1
            return 'Insufficient funds'
        self.transactions.append(Transaction(self.id+1,self.balance,self.balance-amount))
        self.balance = self.balance - amount
        self.id+=1
        return self.balance
...
def make_change(amount, coins):
    ...
    if amount==smallest:
        return [smallest]
    b = make_change(amount-smallest,rest)
    if b == None:
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        b = make_change(coin,self.coins)
        if b!=None:
         for a in b:
                self.coins = remove_one(self.coins,a)
         if coin  not in self.coins.keys():
            self.coins[coin] = 1
         else: 
            self.coins[coin]+=1
         """
